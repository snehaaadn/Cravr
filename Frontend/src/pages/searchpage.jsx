...
const MOCK_RESTAURANTS = [
    { id: 1, name: "The Burger Club", rating: 4.5, time: "25-30 min", price: "$200 for two", cuisine: "American, Fast Food", image: burgerImg, location: "Downtown" },
    { id: 2, name: "Biryani Blues", rating: 4.2, time: "35-40 min", price: "$400 for two", cuisine: "Indian, Mughlai", image: biryaniImg, location: "Westside" },
    { id: 3, name: "Dosa Plaza", rating: 4.8, time: "20-25 min", price: "$150 for two", cuisine: "South Indian", image: dosaImg, location: "Uptown" },
    { id: 4, name: "Pizza Hut", rating: 4.0, time: "30-45 min", price: "$300 for two", cuisine: "Italian, Pizzas", image: pizzaImg, location: "Downtown" },
];

const MOCK_DISHES = [
    { id: 101, name: "Spicy Chicken Burger", restaurant: "The Burger Club", price: 149, rating: 4.5, image: burgerImg, description: "Juicy chicken patty with spicy mayo." },
    { id: 102, name: "Hyderabadi Dum Biryani", restaurant: "Biryani Blues", price: 299, rating: 4.7, image: biryaniImg, description: "Authentic spices cooked with basmati rice." },
    { id: 103, name: "Masala Dosa", restaurant: "Dosa Plaza", price: 120, rating: 4.3, image: dosaImg, description: "Crispy crepe filled with potato masala." },
    { id: 104, name: "Farmhouse Pizza", restaurant: "Pizza Hut", price: 349, rating: 4.1, image: pizzaImg, description: "Loaded with cheese and fresh veggies." },
    { id: 105, name: "Hakka Noodles", restaurant: "China Town", price: 180, rating: 4.0, image: noodlesImg, description: "Wok-tossed noodles with crunchy veggies." },
];

function SearchPage() {
    const [searchParams] = useSearchParams();
    const queryLocation = searchParams.get('pincode'); // e.g. ?pincode=110001
    const queryDish = searchParams.get('q');           // e.g. ?q=burger
    ...
    useEffect(() => {
        setLoading(true);
        // Reset based on URL params logic
        if (queryLocation) setActiveTab('restaurants');
        else if (queryDish) setActiveTab('dishes');

        const fetchData = setTimeout(() => {
            if (activeTab === 'restaurants') {
                // Filter restaurants (Simulating backend search)
                setResults(MOCK_RESTAURANTS); 
            } else {
                // Filter dishes (Simulating backend search)
                const filtered = queryDish 
                    ? MOCK_DISHES.filter(d => d.name.toLowerCase().includes(queryDish.toLowerCase()))
                    : MOCK_DISHES;
                setResults(filtered.length > 0 ? filtered : MOCK_DISHES); // Fallback to all if no match for demo
            }
            setLoading(false);
