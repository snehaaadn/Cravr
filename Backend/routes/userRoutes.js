...
router.route('/cart') // /api/user/cart
    .post('/', paymentCheck, async (req, res) => { // To make order from cart
        const {
            userId,
            restaurantId,
            cartItems,
    ...
    .get('/', async (req, res) => { // Get user cart
    ...
    .delete('/', async (req, res) => { // Remove dish from cart
        const { userID } = req.params;
    ...
    .patch('/', async (req, res) => { // Update dish quantity in cart
        const { userID } = req.params;
